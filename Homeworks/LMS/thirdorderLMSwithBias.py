...
def nextWeights(err, prevW, t_arr, x1, x2):
    newW = prevW + (np.dot(((err)/(pow(Euclidian(x1,x2),3))), t_arr))
    return newW

def Predict(mon_arr, lr, bias):
    ...
    for i in range(1, len(mon_arr), 2):
        ...
        pred = np.dot(w, temp_arr) + bias
        pred_arr[x] = pred
        error = lr * (mon_arr[j] - pred)
        bias += error
...
predictions = Predict(month_arr, lr, bias)
